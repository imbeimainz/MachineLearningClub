...
With estimates $\hat\beta_0$  ,  $\hat\beta_1$, one can make make predictions 
...
$$ 
\text{SE}(\hat\beta_1)^2=\frac{\hat\sigma^2}{\sum_{i=1}^n(x_i-\xbar)^2} \qquad 
\text{SE}(\hat\beta_0)^2=\hat\sigma^2\left[\frac 1 n + \frac{\xbar^2}{\sum_{i=1}^n(x_i-\xbar)^2}\right]
$$
where $\sigma^2:=\text{Var}(\eps)$ is beeing estimated by $\hat\sigma^2:=\frac{(y_i-\hat y_i)^2}{(n-2)}$
...
$$
Then the estimator for the coefficients can be expressed as 

$$ 
\hat \beta = \underbrace{(X^T X)^{-1}X^T }_{\small{\text{ Pseudinverse of }} X} \cdot Y
$$ 
